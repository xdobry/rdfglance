...
use crate::{
    IriIndex,
    domain::{Literal, NodeData, type_index::TypeInstanceIndex},
};
...
pub fn resolve_references(
    ...
    from_type: IriIndex,
    from_predicate: IriIndex,
    to_type: IriIndex,
    to_predicate: IriIndex,
    predicate: IriIndex,
) -> usize {
    ...
    let Some(to_type) = type_instance_index.types.get(&to_type) else {
        return 0;
    };
    let Some(from_type) = type_instance_index.types.get(&from_type) else {
        return 0;
    };

    let to_index: HashMap<IriIndex, IriIndex> = to_type
        .instances
        .par_iter()
        .fold(
            || HashMap::new(),
            |mut local_map, inst_index| {
                if let Some((_iri, node)) = node_data.get_node_by_index(*inst_index) {
                    for (predicate_index, literal) in &node.properties {
                        if *predicate_index == to_predicate {
                            if let Literal::StringShort(str_index) = literal {
                                local_map.insert(*str_index, *inst_index);
                                break;
                            }
                        }
                    }
    ...
    for inst_index in from_type.instances.iter() {
...
mod tests {
    ...
    use super::*;
    use crate::{
        domain::{RdfData, prefix_manager::PrefixManager, reference_resolver, type_index::TypeInstanceIndex},
        integration::rdfwrap::RDFWrap,
    };
        ...
        let from_type = t
            ...
            .find(|t_index| {
                rdf_data
                    .node_data
                    .get_type(**t_index)
                    .map_or(false, |type_iri| type_iri.ends_with("#track"))
            })
            .unwrap();
        let predicate = rdf_data
            .node_data
            .indexers
            .predicate_indexer
            .map
            .iter()
            .find(|(_p_index, pstr)| pstr.ends_with("albumid"))
            .unwrap()
            .0
            .to_usize() as IriIndex;
        let to_type = t
            .types_order
            .iter()
            .find(|t_index| {
                rdf_data
                    .node_data
                    .get_type(**t_index)
                    .map_or(false, |type_iri| type_iri.ends_with("#album"))
            })
            .unwrap();

        let res = reference_resolver::resolve_references(
