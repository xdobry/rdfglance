...
use crate::{drawing::{self, draw_node_label}, graph_styles::{EdgeFont, EdgeStyle, NodeStyle}, graph_view::is_overlapping, layout::{layout_graph_nodes, update_edges_groups, Edge, SortedNodeLayout}, nobject::{IriIndex, LabelContext}, table_view::TypeInstanceIndex, uitools::popup_at, NodeAction, RdfGlanceApp};
    ...
    pub fn show_meta_graph(&mut self, ctx: &egui::Context, ui: &mut egui::Ui) -> NodeAction {
        ...
        ui.horizontal(|ui| {
            if ui.button("Show Meta Graph").clicked() {
                ...
                self.meta_nodes.edges = create_types_layout_edges(&self.meta_nodes, &self.type_index);
                self.ui_state.temperature = 100.0;
            }
            if ui.checkbox(&mut self.ui_state.meta_force_compute_layout, "Force Layout").changed() {
                if self.ui_state.meta_force_compute_layout {
                    self.ui_state.temperature = 100.0;
                }
            }
            ...
            if self.ui_state.meta_compute_layout || self.ui_state.meta_force_compute_layout {
                ...
                self.meta_nodes.positions = positions;
                if (max_move < 0.8 || self.ui_state.temperature<0.5) && !self.ui_state.meta_force_compute_layout {
        ...
        egui::CentralPanel::default().show_inside(ui, |ui| {
            ...
            scene.show(ui, &mut self.graph_state.scene_rect, |ui| {
                ...
                edge_count += self.meta_nodes.edges.len();
                for edge in self.meta_nodes.edges.iter() {
                    let node_layout = &self.meta_nodes.nodes[edge.from];
                    let node_label = || {
                    ...
                    let pos1 = center + self.meta_nodes.positions[edge.from].pos.to_vec2();
                    let p_edge_style = self.visualisation_style.get_edge_syle(edge.predicate);
                    edge_style.color = p_edge_style.color;
                    if edge.from != edge.to {
                        let ref_object = &self.meta_nodes.nodes[edge.to];
                        let pos2 = center + self.meta_nodes.positions[edge.to].pos.to_vec2();
                        drawing::draw_edge(
                            painter,
                            pos1,
                            node_layout.size,
                ...
                if let Some(node_to_drag_index) = &self.ui_state.node_to_drag {
                    if let Some((_node_to_drag, node_pos)) = self.meta_nodes.get_pos(*node_to_drag_index) {
                        self.meta_nodes.positions[node_pos].pos = (mouse_pos - center - self.ui_state.drag_diff.to_vec2()).to_pos2();
                    }
                }
                let mut node_style : NodeStyle = NodeStyle::default();
                for (node_layout, node_position) in self.meta_nodes.nodes.iter_mut().zip(self.meta_nodes.positions.iter()) {
                        ...
                        match node_action {
                            ...
                            TypeNodeContextAction::HideSameInstCount => {
                                if let Some(current_node) = self.type_index.types.get(&current_index) {
                                    let to_remove = self.meta_nodes.nodes.iter().filter(|node| {
...
fn create_types_layout_edges(layout_nodes: &SortedNodeLayout,type_index: &TypeInstanceIndex) -> Vec<Edge> {
    ...
    for (node_pos, node_layout) in layout_nodes.nodes.iter().enumerate() {
            ...
            for (pred_index, ref_characteristics) in &type_data.references {
                for ref_type_iri in &ref_characteristics.types {
                    if *ref_type_iri == node_layout.node_index {
                        ...
                        if let Some((_node,ref_pos)) = layout_nodes.get_pos(*ref_type_iri) {
                            let edge = Edge {
                                from: node_pos,
                                to: ref_pos,
                                predicate: *pred_index,
...
#[cfg(test)]
mod tests {
    use crate::layout::{layout_graph_nodes, NodePosition};
    ...
    fn test_meta_graph()  -> std::io::Result<()> {
        ...
        assert!(vs.meta_nodes.nodes.len() > 0);
        for (type_index, _type_node) in vs.type_index.types.iter() {
            vs.meta_nodes.add_by_index(*type_index);
        }
        ...
        assert!(edges.len() > 0);
        vs.meta_nodes.edges = edges;
        

        let mut positions: Vec<NodePosition> = Vec::with_capacity(vs.meta_nodes.nodes.len());
        for _ in 0..vs.meta_nodes.nodes.len() {
            positions.push(NodePosition::default());
        }
        vs.meta_nodes.positions = positions;

        let (max_move, positions) = layout_graph_nodes(
            &vs.meta_nodes,
            &vs.persistent_data.config_data,
            100.0,
        );
        assert!(max_move > 0.0);
        assert!(positions.len() == vs.meta_nodes.nodes.len());
